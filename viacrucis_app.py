...
# Definición de rutas directas locales (Cero lag, máxima velocidad)
ruta_fondo = "assets/image.png"
ruta_banner = "assets/Presente.png"

# =========================================================================
# 3. CONTROLADOR DE ESTILOS LIMPIO (Corrección de fuentes e íconos)
# =========================================================================
# Separamos el CSS estático para evitar que las llaves de CSS rompan los f-strings de Python
st.markdown("""
...
/* Forzar que los elementos que usen la clase de íconos no rendericen texto común */
...
# Inyectamos el fondo y banner usando strings limpios para evitar colisiones de llaves
st.markdown(f"""
<style>
.stApp {{
...
# =========================================================================
# 4. CONEXIÓN Y LOGICA DE BASE DE DATOS
# =========================================================================
def conectar():
...
# --- FUNCIÓN CORREGIDA DE EXPORTACIÓN PDF (Previene AttributeErrors de nulos) ---
def generar_pdf_reporte(db_conn):
    ...
    res_in = pd.read_sql("SELECT SUM(abono) as total FROM pago_patrocinantes", db_conn)
    total_in = res_in['total'].iloc[0] or 0
    res_out = pd.read_sql("SELECT SUM(monto) as total FROM gastos", db_conn)
    total_out = res_out['total'].iloc[0] or 0
    saldo = total_in - total_out
    
    pdf.set_font("Arial", '', 11)
    pdf.cell(60, 8, f"Total Ingresos: {total_in:,.2f} COP", ln=True)
    pdf.cell(60, 8, f"Total Gastos: {total_out:,.2f} COP", ln=True)
    ...
    for _, fila in df_utileria.iterrows():
        objeto_val = str(fila['objeto']) if fila['objeto'] is not None else "Sin nombre"
        cant_val = str(fila['cantidad']) if fila['cantidad'] is not None else "0"
        desc_val = str(fila['descripcion']) if fila['descripcion'] is not None else "Sin descripción"
        
        pdf.cell(50, 6, objeto_val, border=1)
        pdf.cell(25, 6, cant_val, border=1, align='C')
        pdf.cell(115, 6, desc_val, border=1)
        pdf.ln()
        
    return pdf.output(dest='S').encode('latin-1')

# =========================================================================
# 5. FILTRO Y PROTECCIÓN DE ACCESO (Módulo de Login Seguros)
# =========================================================================
if not st.session_state['autenticado']:
    # Corrección de íconos: usando HTML con la clase correspondiente para que renderice la llave y el candado
    st.markdown("""
    <div class="banner-acceso">
    <h2 class="texto-acceso">Acceso al sistema <span class="material-symbols-outlined">lock</span></h2>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_cen:
        with st.form("login"):
            # Usamos emojis estándar en los textos de Streamlit ya que los Material Symbols directos en etiquetas no se procesan
...
# --- CONTINUACIÓN OPERADOR AUTENTICADO ---
if st.session_state['autenticado']:
    ...
    try:
        ...
        st.sidebar.download_button(
            ...
            data=pdf_data,
            file_name=f"Reporte_Viacrucis2026_{datetime.now().strftime('%Y%m%d')}.pdf",
    ...
    if st.session_state.get('usuario_rol') == 1:
        with tabs[3]:
            ...
            if not st.session_state.get("bloqueo_advertencia", False):
                ...
                # Alertas visuales de confirmación de datos críticas
                st.markdown("""
