...
class PDFReporte(FPDF):
    def header(self):
        ...
        self.set_fill_color(21, 3, 36)
        self.rect(0, 0, 210, 38, 'F')
        self.set_text_color(255, 255, 255)
        self.set_font('Arial', 'B', 18)
        self.cell(0, 12, 'VIACRUCIS 2026 - PATRIMONIO Y GESTION', 0, 1, 'C')
...
def generar_pdf_patrimonio():
    ...
    pdf.set_text_color(21, 3, 36)
    pdf.set_font('Arial', 'B', 14)
    pdf.cell(0, 10, '1. RESUMEN DE ECONOMIA', 0, 1, 'L')
    pdf.line(10, pdf.get_y(), 200, pdf.get_y())
    pdf.ln(4)
    
    res_in = pd.read_sql("SELECT SUM(abono) as total FROM pago_patrocinantes", db_pdf)
    total_in = res_in['total'].iloc[0] or 0
    res_out = pd.read_sql("SELECT SUM(monto) as total FROM gastos", db_pdf)
    total_out = res_out['total'].iloc[0] or 0
    saldo = total_in - total_out
    
    pdf.set_font('Arial', '', 11)
    pdf.set_text_color(0, 0, 0)
    pdf.cell(60, 8, f' Total Ingresos: {total_in:,.2f} COP', 1, 0, 'L')
    pdf.cell(60, 8, f' Total Gastos: {total_out:,.2f} COP', 1, 0, 'L')
    pdf.set_font('Arial', 'B', 11)
    if saldo >= 0:
        pdf.set_text_color(37, 141, 25) # Verde del sistema
    else:
        pdf.set_text_color(255, 124, 112) # Rojo del sistema
    pdf.cell(70, 8, f' Saldo Neto: {saldo:,.2f} COP', 1, 1, 'L')
    pdf.ln(8)
    
    # --- SECCIÓN 2: PERSONAL REGISTRADO ---
    pdf.set_text_color(21, 3, 36)
    pdf.set_font('Arial', 'B', 14)
    pdf.cell(0, 10, '2. REPORTE DE PERSONAL Y PARTICIPANTES', 0, 1, 'L')
    pdf.line(10, pdf.get_y(), 200, pdf.get_y())
    pdf.ln(4)
    
    query_p = """
    SELECT p.Nombre, p.Apellido, p.Edad, r.Descripción AS Rol, pa.`Nombre Parroquia` AS Parroquia
    FROM participantes p
    JOIN parroquia pa ON p.id_parroquia = pa.id_parroquia
    JOIN roles r ON p.id_rol = r.id_rol
    """
    df_p_pdf = pd.read_sql(query_p, db_pdf)
    
    # Encabezados de Tabla Personal
    pdf.set_fill_color(49, 45, 56)
    pdf.set_text_color(255, 255, 255)
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(40, 7, 'Nombre', 1, 0, 'C', True)
    pdf.cell(40, 7, 'Apellido', 1, 0, 'C', True)
    pdf.cell(15, 7, 'Edad', 1, 0, 'C', True)
    pdf.cell(45, 7, 'Rol/Personaje', 1, 0, 'C', True)
    pdf.cell(50, 7, 'Parroquia', 1, 1, 'C', True)
    
    pdf.set_text_color(0, 0, 0)
    pdf.set_font('Arial', '', 9)
    for _, fila in df_p_pdf.iterrows():
        pdf.cell(40, 6, str(fila['Nombre'])[:20], 1, 0, 'L')
        pdf.cell(40, 6, str(fila['Apellido'])[:20], 1, 0, 'L')
        pdf.cell(15, 6, str(fila['Edad']), 1, 0, 'C')
        pdf.cell(45, 6, str(fila['Rol'])[:24], 1, 0, 'L')
        pdf.cell(50, 6, str(fila['Parroquia'])[:26], 1, 1, 'L')
    pdf.ln(8)
    
    # --- SECCIÓN 3: INVENTARIO DE UTILERÍA ---
    pdf.set_text_color(21, 3, 36)
    pdf.set_font('Arial', 'B', 14)
    pdf.cell(0, 10, '3. INVENTARIO GENERAL DE UTILERIA', 0, 1, 'L')
    pdf.line(10, pdf.get_y(), 200, pdf.get_y())
    pdf.ln(4)
    
    df_ut_pdf = pd.read_sql("SELECT objeto, cantidad, descripcion FROM utileria", db_pdf)
    
    # Encabezados de Tabla Utilería
    pdf.set_fill_color(49, 45, 56)
    pdf.set_text_color(255, 255, 255)
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(50, 7, 'Objeto / Elemento', 1, 0, 'C', True)
    pdf.cell(20, 7, 'Cant.', 1, 0, 'C', True)
    pdf.cell(120, 7, 'Descripcion', 1, 1, 'C', True)
    
    pdf.set_text_color(0, 0, 0)
    pdf.set_font('Arial', '', 9)
    for _, fila in df_ut_pdf.iterrows():
        pdf.cell(50, 6, str(fila['objeto'])[:25], 1, 0, 'L')
        pdf.cell(20, 6, str(fila['cantidad']), 1, 0, 'C')
        pdf.cell(120, 6, str(fila['descripcion'])[:70], 1, 1, 'L')
...
try:
    ...
    st.sidebar.error(f"No se pudo preparar el PDF: {e}")
        ...
        if opc == "Gasto Nuevo":
            with st.form("nuevo_gasto"):
                ...
                if st.form_submit_button("Guardar Gasto"):
                    ...
                    db.commit()
                    cur.close()
                    st.success(" ✅  Gasto guardado con  é xito.")
                    st.rerun()
                ...
                if st.button(" 🟢  SÍ, CONFIRMAR Y APLICAR CAMBIOS", use_container_width=True):
                    if datos_nuevos is not None:
                        ...
                        try:
                            ...
                            st.error(f" ❌  Error cr í tico al procesar la actualizaci ó n: {err}")
                        finally:
                            cur.close()
