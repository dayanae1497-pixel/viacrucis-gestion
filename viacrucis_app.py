...
# Procesamos ambas imágenes a Base64 para asegurar que renderice donde sea
...
# Inyectamos el fondo y el banner usando f-string con las variables en Base64 asegurando doble llave {{}} para el CSS
st.markdown(f"""
<style>
.stApp {{
    background-image: linear-gradient(180deg, rgba(50, 19, 84, 0.85) 0%, rgba(28, 9, 51, 0.92) 50%, rgba(13, 2, 26, 0.98) 100%), url('data:image/png;base64,{img_fondo_64}');
...
# --- FUNCIÓN DE EXPORTACIÓN PDF CORREGIDA (Previene fallos por registros nulos) ---
...
if not st.session_state['autenticado']:
    # Corrección de íconos: usando la clase material-symbols-outlined dentro de HTML válido
    st.markdown("""
...
if st.session_state['autenticado']:
    ...
    with tabs[1]:
        ...
        try:
            ...
            # Filtros con flechas de despliegue nativas limpias
    ...
    if st.session_state.get('usuario_rol') == 1:
        with tabs[3]:
            ...
            if not st.session_state.get("bloqueo_advertencia", False):
                ...
                # Alertas de confirmación críticas
                st.markdown("""
