...
def conectar():
    ...
    return mysql.connector.connect(
        ...
        user="root", 
        password="", 
        database="viacrucis 2026" # Asegúrate que el nombre sea igual en tu phpMyAdmin local
    )
...
if st.session_state['usuario_rol'] == 1:
    ...
    # --- Pestaña de Administración (Basado en image_4a1eb9.png) ---
if st.session_state.get('usuario_rol') == 1:
    with tabs[3]: # <-- 4 espacios de sangría (Corrige image_499a5f.png)
        st.header("⚙️ Panel de Control")
        
        try: # Línea 618 (image_49b143.png)
            # 1. Carga de datos de Aiven
            df_p = pd.read_sql("SELECT * FROM participantes", db)
            df_v = pd.read_sql("SELECT * FROM vestuario_final", db)
            
            # 2. El botón con KEY ÚNICO (Corrige image_4a9a9b.png)
            if st.button("📄 Generar Reporte PDF Detallado", key="btn_descarga_final"):
                with st.spinner("Generando reporte estratégico..."):
                    # Llamamos a tu función de PDF corregida
                    pdf_bytes = generar_reporte_completo(df_p, df_v)
                    
                    # Botón de descarga real
                    st.download_button(
                        label="⬇️ Descargar Reporte PDF",
                        data=pdf_bytes,
                        file_name="Reporte_Viacrucis_2026.pdf",
                        mime="application/pdf"
                    )
                    st.balloons()

        except Exception as e: # <-- ESTO CIERRA EL TRY (Corrige image_49ad7f.png)
            st.error(f"Error al conectar o generar: {e}") 
else:
    with tabs[3]:
        st.warning("⛔ Acceso restringido a la coordinación.")

# 1. Asegúrate de tener esta importación al principio de tu archivo
# 1. Asegúrate de que esto esté al puro inicio de tu archivo viacrucis_app.py
# --- Asegúrate de que esto esté al INICIO de tu archivo ---
from fpdf import FPDF

# --- Define la función de PDF una sola vez (fuera de los tabs) ---
def generar_reporte_final(df_p, df_v):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Arial", 'B', 16)
    pdf.cell(40, 10, "Reporte Final Viacrucis 2026")
    pdf.ln(10)
    # Aquí puedes agregar más lógica de diseño si quieres
    return pdf.output()

# --- Lógica dentro de tu interfaz ---
if st.session_state.get('usuario_rol') == 1:
    with tabs[3]: # <-- Verifica que esta línea tenga la sangría correcta (image_499a5f.png)
        st.header("⚙️ Panel de Control")
        
        # Agregamos un bloque try para capturar errores de conexión a Aiven
        try:
            # Cargamos la data necesaria
            df_p = pd.read_sql("SELECT * FROM participantes", db)
            df_v = pd.read_sql("SELECT * FROM vestuario_final", db)
            
            st.info("💡 Haz clic abajo para generar el documento consolidado.")
            
            # EL BOTÓN ÚNICO (Usamos un key diferente para evitar errores: image_4a9a9b.png)
            if st.button("📄 Generar Reporte PDF Detallado", key="btn_final_marzo"):
                with st.spinner("Preparando reporte de pinga..."):
                    # Generamos los bytes directos (Sin .encode() para evitar image_591a1f.png)
                    pdf_bytes = generar_reporte_final(df_p, df_v)
                    
                    # Botón de descarga que aparece solo cuando el PDF está listo
                    st.download_button(
                        label="⬇️ Descargar Reporte PDF",
                        data=pdf_bytes,
                        file_name="Reporte_Viacrucis_Final.pdf",
                        mime="application/pdf"
                    )
                    st.balloons()

        except Exception as e:
            # Si aquí sale 'Parroquia', revisa el nombre de la columna en Aiven (image_590f9f.png)
            st.error(f"Error al conectar o generar: {e}")
else:
    with tabs[3]:
        st.warning("⛔ Acceso restringido a la coordinación.")
